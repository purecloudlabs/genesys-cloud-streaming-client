...
import { IUser, IOrg, IUserQueue, IContext, ICallOptions } from '../../types/types';
...
export function setupStreamingPubsub (topic: string, streamingClient: Client, bulk = true): Observable<any> {
...
export async function getConnectedStreamingClient (authToken?: string, jwt?: string): Promise<Client> {
  const conn = createConnection(authToken);
  await conn.connect();
  return conn;
}

export function createConnection (authToken?: string, jwt?: string): Client {
  const options = {
    authToken,
    logger: console,
    host: getConfig().host,
    ...
    jwt,
    // jid: !jwt ? getContext().user.chat.jabberId : ''
  };
  console.log('Streaming connection options: ', JSON.stringify(options));
  const _client = new (window as any).GenesysCloudStreamingClient(options);
...
export async function testCall (testContext: any, streamingClient: Client, callOptions: ICallOptions) {
  if (!streamingClient) {
...
export async function validateStream (session: GenesysCloudMediaSession, stream: MediaStream, conversationId?: string, shouldAttach = true, reattach?: boolean, descriptor?: string) {
  if (shouldAttach) {
  ...
  logger.log('validating audio stats');
...
export async function validateAudioStats (session: any) {
  let handleStats;
  const statsVerified = new Promise<void>((resolve, reject) => {
    ...
    handleStats = function (stats) {
      logger.log('StatsGatherer audio', stats);
      if (stats.name !== 'getStats' || !stats?.remoteTracks?.length || !stats?.tracks?.length) {
        return;
      }
      try {
...
export async function validateVideoStats (session: GenesysCloudMediaSession) {
  logger.log('starting to validate video stats');
  let handleStats;
  const statsVerified = new Promise<void>((resolve, reject) => {
    handleStats = function (stats) {
      rejectTimeout(reject, 'the session stats to have acceptable video stats', getConfig().validationTimeout);
      logger.log('StatsGatherer Video', stats);
      if (!stats?.remoteTracks?.length) {
        return;
      }
      try {
...
export async function validateVideoStream (session: GenesysCloudMediaSession, stream: MediaStream) {
  ...
  const oStop = videoTrack.stop;

  videoTrack.stop = () => {
    logger.warn('video track stop() called', videoTrack);
    oStop();
  };

  videoTrack.addEventListener('ended', (evt) => {
    logger.warn('video track ended', { evt, videoTrack });
  });
