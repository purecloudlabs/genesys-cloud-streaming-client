...
export class WebrtcExtension extends EventEmitter implements StreamingClientExtension {
  ...
  async handleStanzaInstanceChange (stanzaInstance: NamedAgent) {
    ...
    // because Stanza doesn't know about it). Since we now track those sessions ourselves, we can
    // tell the new Stanza instance about them so those actions will be processed properly.
  ...
  getAllSessions (): IMediaSession[] {
    // jingle.sessions is a Map/object
